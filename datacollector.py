  ...
  """
    ...
    def append(self, frameid, Result):
        ...
        for idx, mot_item in enumerate(rects):
            # self.collector.popitem()
            # print(mot_item)
            ids = int(mot_item[0])
            # print("ids %d idx %d" %(ids,idx))
            if ids not in self.collector:

                self.collector[ids] = copy.deepcopy(self.mots)
            # self.collector[ids]["frames"].clear()
            self.collector[ids]["frames"].append(frameid)
            # self.collector[ids]["rects"].clear()
            # print(len(self.collector[ids]["rects"]))
                # print(self.collector[ids]["rects"])
            ...
            if reid_res:
                ...
                self.collector[ids]["features"].append(reid_res['features'][
                    idx])
                if len(self.collector[ids]["features"])>5:
                    self.collector[ids]["features"].pop()
                self.collector[ids]["qualities"].append(reid_res['qualities'][
                    idx])
                if len(self.collector[ids]["qualities"])>5:
                    self.collector[ids]["qualities"].pop()
               
            if vehicleplate_res and vehicleplate_res['plate'][idx] != "":
                self.collector[ids]["vehicleplate"].append(vehicleplate_res[
                    'plate'][idx])
    def update(self, frameid, Result):
