...
from python.visualize import (
    visualize_box_mask,
    visualize_attr,
    visualize_pose,
    visualize_action,
    visualize_vehicleplate,
    visualize_vehiclepress,
    visualize_lane,
    visualize_vehicle_retrograde,
)
...
from pphuman.action_infer import (
    SkeletonActionRecognizer,
    DetActionRecognizer,
    ClsActionRecognizer,
)
...
class Pipeline(object):
    ...
    def _parse_input(
        self, image_file, image_dir, video_file, video_dir, camera_id, rtsp
    ):
        ...
        if image_file is not None or image_dir is not None:
            ...
            assert (
                os.path.exists(video_file) or "rtsp" in video_file
    ...
    def run_multithreads(self):
        ...
        if self.multi_camera:
            ...
            for idx, (predictor, input) in enumerate(zip(self.predictor, self.input)):
                threadQueue = queue.Queue(2)
                frameQueue = queue.Queue(2)
                thread = threading.Thread(
                    name=str(idx).zfill(3),
                    target=predictor.run,
                    args=(input, idx, threadQueue, frameQueue),
                )
                threads.append(thread)
                threadQueues.append(threadQueue)
                frameQueues.append(frameQueue)
            ...
            while True:
                ...
                for threadRes, frameRes in zip(threadQueues, frameQueues):
                    # print("the res size%d" %threadRes.qsize())
                    # print("the frame size%d" %frameRes.qsize())
                    collector_data = threadRes.get()
                    frame_res.append(frameRes.get())
                    # print(result["res"])
...
class PipePredictor(object):
    ...
    def __init__(self, args, cfg, is_video=True, multi_camera=False):
        ...
        self.with_mot = (
            cfg.get("MOT", False)["enable"] if cfg.get("MOT", False) else False
        )
        self.with_human_attr = (
            cfg.get("ATTR", False)["enable"] if cfg.get("ATTR", False) else False
        )
        if self.with_mot:
        ...
        # only for pphuman
        self.with_skeleton_action = (
            cfg.get("SKELETON_ACTION", False)["enable"]
            if cfg.get("SKELETON_ACTION", False)
            else False
        )
        self.with_video_action = (
            cfg.get("VIDEO_ACTION", False)["enable"]
            if cfg.get("VIDEO_ACTION", False)
            else False
        )
        self.with_idbased_detaction = (
            cfg.get("ID_BASED_DETACTION", False)["enable"]
            if cfg.get("ID_BASED_DETACTION", False)
            else False
        )
        self.with_idbased_clsaction = (
            cfg.get("ID_BASED_CLSACTION", False)["enable"]
            if cfg.get("ID_BASED_CLSACTION", False)
            else False
        )
        self.with_mtmct = (
            cfg.get("REID", False)["enable"] if cfg.get("REID", False) else False
        )
        ...
        # only for ppvehicle
        self.with_vehicleplate = (
            cfg.get("VEHICLE_PLATE", False)["enable"]
            if cfg.get("VEHICLE_PLATE", False)
            else False
        )
        if self.with_vehicleplate:
            print("Vehicle Plate Recognition enabled")

        self.with_vehicle_attr = (
            cfg.get("VEHICLE_ATTR", False)["enable"]
            if cfg.get("VEHICLE_ATTR", False)
            else False
        )
        if self.with_vehicle_attr:
            print("Vehicle Attribute Recognition enabled")

        self.with_vehicle_press = (
            cfg.get("VEHICLE_PRESSING", False)["enable"]
            if cfg.get("VEHICLE_PRESSING", False)
            else False
        )
        if self.with_vehicle_press:
            print("Vehicle Pressing Recognition enabled")

        self.with_vehicle_retrograde = (
            cfg.get("VEHICLE_RETROGRADE", False)["enable"]
            if cfg.get("VEHICLE_RETROGRADE", False)
            else False
        )
        ...
        if self.with_vehicle_press:
            ...
            self.modebase[basemode] = True
            self.vehicle_press_predictor = VehiclePressingRecognizer(vehiclepress_cfg)
        ...
        if not is_video:
            ...
            if (
                self.with_mot
                or self.modebase["idbased"]
                or self.modebase["skeletonbased"]
            ):
    ...
    def run(self, input, thread_idx=0, threadQueue=None, frameQueue=None):
        if self.is_video:
            self.predict_video(
                input,
    ...
    def predict_video(
        self, video_file, thread_idx=0, threadQueue=None, frameQueue=None
    ):
        # mot
        # mot -> attr
        # mot -> pose -> action
        ...
        if len(self.pushurl) > 0:
            ...
            video_out_name = (
                "output"
                if (self.file_name is None or type(self.file_name) == int)
                else self.file_name
            )
            if type(video_file) == str and "rtsp" in video_file:
                video_out_name = (
                    video_out_name + "_t" + str(thread_idx).zfill(2) + "_rtsp"
                )
            if not os.path.exists(self.output_dir):
                os.makedirs(self.output_dir)
            out_path = os.path.join(self.output_dir, video_out_name + ".flv")
        ...
        if (
            ...
            or self.do_break_in_counting
            or self.illegal_parking_time != -1
        ):
        ...
        object_in_region_info = {}  # store info for vehicle parking in region
        illegal_parking_dict = None
        cars_count = 0
        retrograde_traj_len = 0
        framequeue = queue.Queue(5)

        thread = threading.Thread(target=self.capturevideo, args=(capture, framequeue))
        thread.start()
        time.sleep(1)
        ...
        while not framequeue.empty():
            ...
            if self.modebase["idbased"] or self.modebase["skeletonbased"]:
                ...
                if (
                    mot_skip_frame_num > 1
                    and frame_id > 0
                    and frame_id % mot_skip_frame_num > 0
                ):
                ...
                if self.with_vehicle_press or self.with_vehicle_retrograde:
                    if (
                        frame_id == 0
                        or cars_count == 0
                        or cars_count > len(mot_res["boxes"])
                    ):
                        if frame_id > self.warmup_frame:
                ...
                if self.with_skeleton_action:
                    ...
                    if state:
                        ...
                        collected_keypoint = (
                            self.kpt_buff.get_collected_keypoint()
                        )  # reoragnize kpt output with ID
                        skeleton_action_input = parse_mot_keypoint(
                            collected_keypoint, self.coord_size
                        )
                        skeleton_action_res = (
                            self.skeleton_action_predictor.predict_skeleton_with_mot(
                    ...
                    if self.cfg["visual"]:
                        self.skeleton_action_visual_helper.update(skeleton_action_res)
                ...
                if self.with_mtmct:
                    (
                        ...
                        img_qualities,
                        rects,
                    ) = self.reid_predictor.crop_image_with_mot(frame_rgb, mot_res)
                    if frame_id > self.warmup_frame:
            ...
            if self.with_vehicle_retrograde:
                ...
                if retrograde_traj_len == frame_len:
                    retrograde_mot_res = copy.deepcopy(self.pipeline_res.get("mot"))
                    retrograde_lanes = copy.deepcopy(self.pipeline_res.get("lanes"))
                    ...
                    (
                        retrograde_res,
                        fence_line,
                    ) = self.vehicle_retrograde_predictor.mot_run(
                        ...
                        if retrograde_res is not None:
                            for retrograde_res_id in retrograde_res:
                                if retrograde_res_id not in retrograde_res_update:
                                    retrograde_res_update.append(retrograde_res_id)
                    else:
                    ...
                    self.pipeline_res.update(retrograde_res_dict, "vehicle_retrograde")

                    if frame_id > self.warmup_frame:
                        self.pipe_timer.module_time["vehicle_retrograde"].end()

                    retrograde_traj_len = 0
            collectorRtsp = DataCollector()
            # resRtsp = {"res": [],"frame": []}
            collectorRtsp.append(frame_id, self.pipeline_res)
            self.collector.append(frame_id, self.pipeline_res)
            # if self.collector.collector.
            # self.collector.collector.pop()
            # print(self.collector.get_res())
            # resRtsp["res"].append()
            # resRtsp['frame'].append(frame_rgb)
            # if threadQueue.full():
            #     threadQueue.queue.clear()
            """
            获取预测结果
            """
            # print(self.collector.collector.keys())
            # print("collect")
            print(collectorRtsp.collector.keys())

            # print("!!!!!!!!!!")
            threadQueue.put(collectorRtsp.get_res())
    ...
    def visualize_video(
        ...
        if kpt_res is not None:
            image = visualize_pose(
                image, kpt_res, visual_thresh=self.cfg["kpt_thresh"], returnimg=True
            )
        ...
        if vehicle_retrograde_res is not None:
            ...
            image = visualize_vehicle_retrograde(
                image, mot_retrograde_res, vehicle_retrograde_res
            )
            image = np.array(image)
        ...
        if len(visual_helper_for_display) > 0:
            image = visualize_action(
                image, mot_res["boxes"], visual_helper_for_display, action_to_display
            )
    ...
    def visualize_image(self, im_files, images, result):
        ...
        for i, (im_file, im) in enumerate(zip(im_files, images)):
            if det_res is not None:
                ...
                det_res_i["boxes"] = det_res["boxes"][
                    start_idx : start_idx + boxes_num_i, :
                ]
                im = visualize_box_mask(
                    im, det_res_i, labels=["target"], threshold=self.cfg["crop_thresh"]
                )
                im = np.ascontiguousarray(np.copy(im))
                im = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
            if human_attr_res is not None:
                human_attr_res_i = human_attr_res["output"][
                    start_idx : start_idx + boxes_num_i
                ]
            ...
            if vehicleplate_res is not None:
                ...
                det_res_i["boxes"][:, 4:6] = (
                    det_res_i["boxes"][:, 4:6] - det_res_i["boxes"][:, 2:4]
                )
